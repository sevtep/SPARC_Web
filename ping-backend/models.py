...
class Class(Base):
    ...
    teacher = relationship(
        "User", back_populates="classes_teaching", foreign_keys=[teacher_id]
    )
...
class BehaviorData(Base):
    ...
    )  # 'keypress', 'click', 'objective_complete', etc.
...
class UserModuleCompletion(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "module_id", name="uq_user_module_completions"),
    )
...
class ExternalAccount(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "provider", "external_user_id", name="uq_external_accounts_provider_user"
        ),
    )
